  ...
  return (
    <aside className="w-20 md:w-24 bg-gray-50 border-l border-gray-200 h-full flex flex-col">
      ...
      <div className="px-2 md:px-3 py-2 text-[10px] md:text-[11px] font-medium text-gray-600 text-center">
        <span className="hidden md:inline">{t('thumbnail.header')}</span>
      ...
      <div className="flex-1 overflow-y-auto px-2 md:px-3 pb-3 space-y-2">
        ...
        {originalImage && (
          <button
            ...
            title={t('thumbnail.original')}
          >
            <div className="relative w-full aspect-[3/4] bg-white">
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src={originalImage} alt={t('thumbnail.original')} className="absolute inset-0 w-full h-full object-cover" />
            </div>
            <span className="absolute top-1 left-1 bg-blue-600 text-white text-[10px] px-1 rounded">{t('thumbnail.original')}</span>
            {selectedIndex === -1 && (
        ...
        {history.map((item, idx) => (
          <div key={item.id} className="relative">
            <button
              ...
              title={t('thumbnail.history_item', { index: String(idx + 1) })}
            >
              <div className="relative w-full aspect-[3/4] bg-white">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src={item.imageUrl} alt={t('thumbnail.edit_alt', { index: String(idx + 1) })} className="absolute inset-0 w-full h-full object-cover" />
              </div>
              <span className="absolute top-1 left-1 bg-purple-600 text-white text-[10px] px-1 rounded">{t('thumbnail.history_item', { index: String(idx + 1) })}</span>
              {selectedIndex === idx && (
